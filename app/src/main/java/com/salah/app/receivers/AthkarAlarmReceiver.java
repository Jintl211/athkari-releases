...
public class AthkarAlarmReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        // تشغيل الصوت عبر الخدمة حتى يمكن إيقافه
        startAthkarService(context, type);
        ...
        if ("morning".equals(type) && s.morningAthkarEnabled) {
            AlarmScheduler.scheduleAthkar(context, "morning", 6, 0); // الساعة 6:00 ص
        } else if ("sleep".equals(type)) {
            // أذكار النوم الساعة 9:00 م
            AlarmScheduler.scheduleAthkar(context, "sleep", 21, 0);
        } else if ("evening".equals(type) && s.eveningAthkarEnabled) {
            AlarmScheduler.scheduleAthkar(context, "evening", s.eveningAthkarHour, s.eveningAthkarMinute);
        }
    }

    private void startAthkarService(Context context, String type) {
        try {
            int soundRes = 0;
            if ("morning".equals(type)) soundRes = com.salah.app.R.raw.adhkar_morning;
            else if ("sleep".equals(type)) soundRes = com.salah.app.R.raw.adhkar_sleep;
            else if ("evening".equals(type)) soundRes = com.salah.app.R.raw.adhkar_morning;
            if (soundRes != 0) {
                Intent serviceIntent = new Intent(context, com.salah.app.services.AthkarSoundService.class);
                serviceIntent.putExtra("sound_res", soundRes);
                serviceIntent.putExtra("athkar_type", type);
            ...
            android.util.Log.e("AthkarAlarm", "Failed to start service", e);
        }
    }
